...
class AdminDashboard {
    ...
    loadViewData(view) {
        switch(view) {
            case 'dashboard':
                // Dashboard overview - already loaded
                break;
            case 'profile':
                this.loadProfile();
                break;
            ...
            case 'messages':
                ...
                break;
            case 'contact':
                // Contact/Messages same as messages
                this.loadMessages();
                break;
            case 'settings':
                // Settings section
    ...
    saveAboutInfo() {
        ...
        this.saveData('about_info', aboutInfo);
        this.updateStats();
    }
    ...
    loadAboutInfo() {
        ...
        if (aboutInfo) {
            ...
            if (form) {
                // Load description
                if (form.description) form.description.value = aboutInfo.description || '';
                
                // Load stats
                if (aboutInfo.stats) {
                    if (form.projects_count) form.projects_count.value = aboutInfo.stats.projects ? aboutInfo.stats.projects.replace('+', '') : '';
                    if (form.years_experience) form.years_experience.value = aboutInfo.stats.experience ? aboutInfo.stats.experience.replace('+', '') : '';
                    if (form.technologies_count) form.technologies_count.value = aboutInfo.stats.technologies ? aboutInfo.stats.technologies.replace('+', '') : '';
                    if (form.satisfaction_rate) form.satisfaction_rate.value = aboutInfo.stats.satisfaction ? aboutInfo.stats.satisfaction.replace('%', '') : '';
                }
                
                // Load skill categories
                if (aboutInfo.skillCategories) {
                    if (form.aiml_title) form.aiml_title.value = aboutInfo.skillCategories.aiml?.title || '';
                    if (form.aiml_tags) form.aiml_tags.value = aboutInfo.skillCategories.aiml?.tags?.join(', ') || '';
                    if (form.dev_title) form.dev_title.value = aboutInfo.skillCategories.development?.title || '';
                    if (form.dev_tags) form.dev_tags.value = aboutInfo.skillCategories.development?.tags?.join(', ') || '';
                    if (form.cloud_title) form.cloud_title.value = aboutInfo.skillCategories.cloud?.title || '';
                    if (form.cloud_tags) form.cloud_tags.value = aboutInfo.skillCategories.cloud?.tags?.join(', ') || '';
                }
                
                // Load resume info
                if (aboutInfo.resume) {
                    if (form.resume_link) form.resume_link.value = aboutInfo.resume.link || '';
                    if (form.resume_text) form.resume_text.value = aboutInfo.resume.text || '';
                }
            }
        }
    }
    ...
    logout() {
        if (confirm('Are you sure you want to logout?')) {
            // Clear all session and local storage for admin
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (!isAuthenticated || !loginTime) {
            // No authentication, redirect immediately
