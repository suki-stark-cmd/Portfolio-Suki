// Supabase-powered Portfolio Data Manager
import { supabaseService } from './supabase-config.js';

class SupabasePortfolioDataManager {
    constructor() {
    ...
    setupRealtimeListeners() {
        ...
        this.supabase.subscribeToTable('personal_info', (result) => {
        ...
        this.supabase.subscribeToTable('about_info', (result) => {
            if (result.success && result.data) {
        ...
        this.supabase.subscribeToTable('projects', async (result) => {
            if (result.success) {
                // Reload all projects when any change occurs
        ...
        this.supabase.subscribeToTable('skills', async (result) => {
            if (result.success) {
                // Reload all skills when any change occurs
        ...
        this.supabase.subscribeToTable('experience', async (result) => {
            if (result.success) {
                // Reload all experience when any change occurs
                const experience = await this.supabase.getExperience();
    ...
    // Load initial portfolio data from Supabase
    async loadPortfolioData() {
        try {
            ...
            // Load personal info
            const personalInfo = await this.supabase.getPersonalInfo();
            if (personalInfo.success && personalInfo.data) {
                this.updatePersonalInfo(personalInfo.data);
            }

            // Load about info
            const aboutInfo = await this.supabase.getAboutInfo();
    ...
    updateAboutInfo(aboutInfo) {
        ...
        if (aboutInfo.skill_categories) {
            this.updateSkillCategoriesPreview(aboutInfo.skill_categories);
        }
    ...
    async handleContactFormSubmission(formData) {
        try {
            ...
            const result = await this.supabase.addMessage(messageData);
    ...
    // Load default content if Supabase fails
    loadDefaultContent() {
    ...
    showConnectionStatus(type = 'success', message = 'Connected to Supabase') {
        let statusIndicator = document.getElementById('supabase-status');
        
        if (!statusIndicator) {
            statusIndicator = document.createElement('div');
            statusIndicator.id = 'supabase-status';
            statusIndicator.className = 'supabase-status hidden';
            document.body.appendChild(statusIndicator);
        }

        const iconClass = type === 'success' ? 'fas fa-database' : 'fas fa-exclamation-triangle';
...
// Initialize Supabase Portfolio Data Manager
document.addEventListener('DOMContentLoaded', () => {
    // Initialize Supabase portfolio manager
    window.portfolioDataManager = new SupabasePortfolioDataManager();
